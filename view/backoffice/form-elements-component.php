            ...
            <div class="pcoded-main-container">
                <div class="pcoded-wrapper">
                    ...
                    <div class="pcoded-content">
                        ...
                        <script>
                        // Fonction de validation pour le nom du sponsor
                        function validateNomSp() {
                            const nomSp = document.getElementById('nom_sp').value;
                            const nomSpError = document.getElementById('nom_sp_error');
                            const regex = /^[a-zA-Z\s]+$/;

                            if (nomSp.trim() === "" || !regex.test(nomSp) || nomSp.length < 3) {
                                nomSpError.textContent =
                                    "Le nom du sponsor doit être une chaîne de caractères et contenir au moins 3 caractères.";
                                nomSpError.style.color = "red";
                            } else {
                                nomSpError.textContent = "";
                            }
                        }
                        ...
                        function validateLogo() {
                            ...
                            if (!logo) {
                                logoError.textContent = "Le logo est obligatoire.";
                                logoError.style.color = "red";
                            } else if (!logo.type.startsWith('image/')) {
                                logoError.textContent = "Le fichier doit être une image.";
                                logoError.style.color = "red";
                            } else {
                        ...
                        // Fonction de validation pour le website
                        function validateWebsite() {
                            ...
                            if (website.trim() === "" || !regex.test(website)) {
                                websiteError.textContent = "Veuillez entrer un site Web valide.";
                        ...
                        document.getElementById("sponsorForm").addEventListener("submit", function(event) {
                            ...
                            // Vérifier si des erreurs existent avant d'envoyer
                            const nomSpError = document.getElementById('nom_sp_error').textContent;
                            const logoError = document.getElementById('logo_error').textContent;
                            const websiteError = document.getElementById('website_error').textContent;
                            const descriptionError = document.getElementById('description_error').textContent;

                            // Si une erreur existe, on bloque la soumission
                            if (nomSpError || logoError || websiteError || descriptionError) {
                                event.preventDefault();
                                alert('Veuillez corriger les erreurs avant de soumettre le formulaire.');
                            }
                        });
